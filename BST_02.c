...
bool searchBst(struct bstNode *node, int element, int level)
{
    ...
    if (element <= node->data)
        return searchBst(node->left, element, level + 1);
...
int main()
{
    ...
    while (1)
    {
        ...
        case 4:
            findMin(root);
            printf("The Samllest Element is %d with node adress of %d\n", findMin(root)->data, findMin(root));
            break;
        case 5:
            findMax(root);
            printf("The Largest Element is %d with node adress of %d\n", findMax(root)->data, findMax(root));
            break;
        case 6:
            printf("The height of the the BST is %d\n", findHeight(root));
            break;
        default:
